    ...
    <div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:8px;margin-bottom:10px;scrollbar-width:none">
        @foreach ($stages as $key => $label)
            <a href="{{ route('fixtures.index', ['stage' => $key]) }}"
               style="white-space:nowrap;padding:6px 14px;border-radius:20px;font-size:12px;font-weight:600;text-decoration:none;border:1px solid;flex-shrink:0;transition:all 0.2s;
                      {{ $stage === $key ? 'background:#00ff87;color:#0a0f1e;border-color:#00ff87' : 'background:transparent;color:#64748b;border-color:#1e2d45' }}">
                {{ $label }}
            </a>
        @endforeach
    </div>
    ...
    @if ($stage === 'group')
        <div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:8px;margin-bottom:16px;scrollbar-width:none">
            <a href="{{ route('fixtures.index', ['stage' => 'group']) }}"
               style="white-space:nowrap;padding:5px 12px;border-radius:20px;font-size:11px;font-weight:700;text-decoration:none;border:1px solid;flex-shrink:0;
                      {{ !$group ? 'background:#f0f4f8;color:#0a0f1e;border-color:#f0f4f8' : 'background:transparent;color:#64748b;border-color:#1e2d45' }}">
                Semua
            </a>
            @foreach ($groups as $key => $label)
                <a href="{{ route('fixtures.index', ['stage' => 'group', 'group' => $key]) }}"
                   style="white-space:nowrap;padding:5px 12px;border-radius:20px;font-size:11px;font-weight:700;text-decoration:none;border:1px solid;flex-shrink:0;
                          {{ $group === $key ? 'background:#f0f4f8;color:#0a0f1e;border-color:#f0f4f8' : 'background:transparent;color:#64748b;border-color:#1e2d45' }}">
                    {{ $label }}
                </a>
            @endforeach
        </div>
