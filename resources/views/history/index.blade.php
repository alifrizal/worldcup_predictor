<x-app-layout>
    ...
    <div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:8px;margin-bottom:8px;scrollbar-width:none">
        ...
        <a href="{{ route('history.index', array_filter(['result_type'=>$key,'stage'=>$stage,'group'=>$group])) }}"
            @style([ 'white-space:nowrap; padding:6px 12px; border-radius:20px; font-size:11px; font-weight:700; text-decoration:none; border:1px solid; flex-shrink:0;' , 'background:#f0f4f8; color:#0a0f1e; border-color:#f0f4f8'=> $resultType === $key,
            'background:transparent; color:#64748b; border-color:#1e2d45' => $resultType !== $key,
            ])
    ...
    <div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:8px;margin-bottom:16px;scrollbar-width:none">
        <a href="{{ route('history.index', array_filter(['result_type'=>$resultType])) }}"
            @style([ 'white-space:nowrap; padding:5px 12px; border-radius:20px; font-size:11px; font-weight:600; text-decoration:none; border:1px solid; flex-shrink:0;' , 'background:#6366f1; color:#fff; border-color:#6366f1'=> !$stage,
            'background:transparent; color:#64748b; border-color:#1e2d45' => $stage,
            ])
            Semua Stage
        </a>
        @foreach ($stages as $key => $label)
        <a href="{{ route('history.index', array_filter(['result_type'=>$resultType,'stage'=>$key])) }}"
            @style([ 'white-space:nowrap; padding:5px 12px; border-radius:20px; font-size:11px; font-weight:600; text-decoration:none; border:1px solid; flex-shrink:0;' , 'background:#6366f1; color:#fff; border-color:#6366f1'=> $stage === $key,
